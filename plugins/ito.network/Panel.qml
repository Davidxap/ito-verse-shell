...
Panel {
  ...
  // Ito-verse: one standard glyph size for every bar icon, and the number beside it.
  Ito.ItoConfig {
    id: itoCfg
    path: Qt.resolvedUrl("../../bar/modules/ito-style.json").toString().replace("file://", "")
  }
  // qs.Ui.Panel carries no barSize (BarWidget does), so read it off the host.
  readonly property int barSize: bar ? bar.barSize : 46
  readonly property int glyphSize: Math.round(root.barSize * itoCfg.get("iconScale", 0.62))
  readonly property bool showValues: itoCfg.get("showValues", true)
  ...
  // Ito skin: theme artwork mirroring the connectionIcon bands above.
  // Kept in sync by construction — same kind/strength bands, with the
  // in-flight connect/scan state surfaced as the connecting art.
  // The engraved wifi set: four arcs' worth of signal, a plug for a wired link, a slashed one for none.
  readonly property url instruments: Qt.resolvedUrl("../../bar/modules/ito-art/instruments/")
  // Which drawing the bar shows for the network (Icons page): "auto" is what is real, the port on a cable and the
  // arcs on Wi-Fi; "wifi" always draws the arcs (full on a cable); "cable" always draws the port.
  readonly property string netIcon: String(itoCfg.get("netIcon", "auto"))
  readonly property var netSources: {
    if (netIcon === "wifi" && kind !== "wifi" && kind !== "") return [instruments + "net-3.png"]
    if (netIcon === "cable" && kind === "wifi") return [instruments + "net-wired.png"]
    if (kind === "ethernet") return [instruments + "net-wired.png"]
    if (kind !== "wifi") return [instruments + "net-off.png"]
    if (scanning || actionKind === "connect") return [instruments + "net-1.png", instruments + "net-2.png"]
    return [instruments + "net-0.png", instruments + "net-1.png",
            instruments + "net-2.png", instruments + "net-3.png"]
  }
  readonly property real netLevel: kind === "wifi" ? Math.max(0, Math.min(1, signalStrength / 100)) : 1

  function wifiArt() {
    if (scanning || actionKind === "connect") return "assets/wifi-connecting.png"
    if (kind === "wifi") {
      var index = Math.max(0, Math.min(4, Math.ceil(signalStrength / 20) - 1))
      return index <= 1 ? "assets/wifi-weak.png" : "assets/wifi.png"
    }
    if (kind === "ethernet") return "assets/wifi.png"
    return "assets/wifi-off.png"
  }
  ...
  BarIconButton {
    id: button
    opticalSize: root.glyphSize
    slotSize: root.glyphSize + 8
    anchors.fill: parent
    bar: root.bar
    text: ""
    iconComponent: wifiArtComponent
  ...
  // Drawn, not pictured: the sheet's small utility icons are crude beside the rest of the bar, so this
  // one is engraved in the same hand as the tray jar and the brain (see scripts/gen-premium.py).
  // Drawn, not pictured: the sheet's small utility icons are crude beside the rest of the bar, so this one
  // is engraved in the same hand as the tray jar and the brain (see scripts/gen-premium.py).
  Component {
    id: wifiArtComponent
    Ito.ItoArt {
  ...
  KeyboardPanel {
    ...
    contentHeight: PopupSize.popupHeight(panel, column.implicitHeight + itoCfg.panelHeadroom + itoCfg.panelFootroom)
    ...
    Ito.ItoAutoClose {
        ...
        hold: root.passwordSsid !== ""
        seconds: Number(itoCfg.get("popupSeconds", 3))
        onExpired: root.close()
    }
    ...
    Column {
      ...
      Item {
        ...
        // Our own signal arcs (the same drawing as the bar), not a font glyph.
        Ito.ItoArt {
          id: heroIcon
          palette: itoCfg
          size: Style.space(40)
  ...
  component NetworkRow: CursorSurface {
    ...
    Item {
      ...
      // The signal as our arcs: one to three lit, the rest faint.
      Ito.ItoImage {
        id: networkIcon
        palette: itoCfg
        readonly property int arcs: row.net ? Math.max(0, Math.min(3, Math.ceil((row.net.signal || 0) / 25))) : 0
        readonly property real implicitHeight: height
        width: Style.space(24)
        height: width
        source: root.instruments + "net-" + arcs + ".png"
        fillMode: Image.PreserveAspectFit
        smooth: true
        mipmap: true
        opacity: row.statusColor === root.bar.foreground ? 1 : 0.9
        anchors.left: parent.left
        anchors.verticalCenter: parent.verticalCenter
      }
