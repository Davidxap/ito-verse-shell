...
Panel {
  ...
  readonly property bool sideBar: bar ? bar.vertical === true : false
  readonly property int glyphSize: Math.round(root.barSize * itoCfg.get("iconScale", 0.62))
  readonly property bool showValues: itoCfg.get("showValues", true) && itoCfg.get("valueStyle", "percent") !== "off"
    && itoCfg.get("content:ito.audio", "both") !== "icon"
  ...
  // Ito skin: theme artwork mirroring the outputIcon bands above.
  // Kept in sync by construction — same thresholds, same mute logic.
  function outputArt() {
    if (!sink || !sink.audio) return "assets/volume-muted.png"
    if (outputMuted) return "assets/volume-muted.png"
    if (outputVolume >= 0.67) return "assets/volume-high.png"
    if (outputVolume >= 0.34) return "assets/volume.png"
    if (outputVolume > 0) return "assets/volume-low.png"
    return "assets/volume-muted.png"
  }
  ...
  // On a side bar the reading goes under the icon, both centred, instead of beside it where there is no room.
  implicitWidth: root.sideBar ? Math.max(button.implicitWidth, valueText.visible ? valueText.implicitWidth : 0)
                               : button.implicitWidth + (valueText.visible ? valueText.implicitWidth + 2 : 0)
  implicitHeight: root.sideBar ? button.implicitHeight + (valueText.visible ? valueText.implicitHeight : 0)
  ...
  BarIconButton {
    ...
    anchors.left: root.sideBar ? undefined : parent.left
    anchors.horizontalCenter: root.sideBar ? parent.horizontalCenter : undefined
    anchors.top: parent.top
    anchors.bottom: root.sideBar ? undefined : parent.bottom
    bar: root.bar
    text: ""
    iconComponent: audioArt
  ...
  Ito.ItoValue {
    id: valueText
    anchors.left: root.sideBar ? undefined : button.right
    anchors.leftMargin: root.sideBar ? 0 : -2
    anchors.verticalCenter: root.sideBar ? undefined : parent.verticalCenter
    anchors.horizontalCenter: root.sideBar ? parent.horizontalCenter : undefined
    anchors.top: root.sideBar ? button.bottom : undefined
    visible: root.showValues && root.hasOutput && !root.outputMuted
    barSize: root.barSize
    label: "VOL"
    vertical: root.sideBar
    hot: root.outputVolume > 1.0
    value: Math.round(root.outputVolume * 100)
  }
  ...
  Component {
    ...
    Ito.ItoArt {
      ...
      sources: !root.hasOutput || root.outputMuted
        ...
        : [instruments + "vol-0.png", instruments + "vol-1.png",
           instruments + "vol-2.png", instruments + "vol-3.png"]
