...
Panel {
  ...
  // Ito skin: theme artwork mirroring the icon bands above.
  function btArt() {
    if (!adapter) return "assets/bluetooth-off.png"
    if (!adapter.enabled) return "assets/bluetooth-off.png"
    if (connectedDevices.length > 0) return "assets/bluetooth.png"
    return "assets/bluetooth-connecting.png"
  }
  ...
  KeyboardPanel {
    ...
    Ito.ItoPanelFrame { cfg: itoCfg; emblem: "rune"; opened: root.opened; memos: ["Not everything paired is still alive.", "Two things, bound. Neither can leave.", "It follows you back."] }
