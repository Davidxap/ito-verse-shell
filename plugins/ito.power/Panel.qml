...
Panel {
  ...
  // Ito skin: theme artwork mirroring the batteryIcon tenths bands above
  // (index = floor(fraction * 10), clamped 0..9).
  function batteryArt() {
    var index = Math.max(0, Math.min(9, Math.floor(batteryFraction * 10)))
    if (index >= 9) return "assets/battery-full.png"
    if (index >= 7) return "assets/battery-75.png"
    if (index >= 5) return "assets/battery-50.png"
    if (index >= 3) return "assets/battery-25.png"
    if (index >= 1) return "assets/battery-low.png"
    return "assets/battery-critical.png"
  }
  ...
  BarIconButton {
    ...
    anchors.fill: parent
    bar: root.bar
    // Percentage label kept as stock shows it; only the glyph becomes art
    // (iconComponent hides the stock glyph, so the art component repaints
    // the label beside the artwork itself).
    text: root.showPercentage && !vertical
      ? Math.round(root.batteryFraction * 100) + "%"
      : ""
    // Null with no battery, like the stock empty icon: keeps the widget
    // hidden instead of painting an art slot for missing hardware.
    iconComponent: root.batteryPresent ? powerArtComponent : null
    slotSize: root.glyphSize + 8 + (root.showPercentage && !vertical ? 26 : 0)
    tooltipText: ""
  ...
  Component {
    ...
    Row {
      ...
      Ito.ItoArt {
        // The engraved cell: blood standing in it to the charge, a bolt across it while it fills.
        readonly property url instruments: Qt.resolvedUrl("../../bar/modules/ito-art/instruments/")
        readonly property string tail: root.discharging ? "" : "-charge"
        anchors.verticalCenter: parent.verticalCenter
        sources: root.discharging
          ? [instruments + "bat-0.png", instruments + "bat-1.png", instruments + "bat-2.png",
             instruments + "bat-3.png", instruments + "bat-4.png"]
          : [instruments + "bat-charge-1.png", instruments + "bat-charge-2.png",
             instruments + "bat-charge-3.png", instruments + "bat-charge-4.png"]
        value: Math.max(0, Math.min(1, root.batteryFraction))
        size: root.glyphSize
        palette: itoCfg
      }
    }
  ...
  KeyboardPanel {
    ...
    Ito.ItoPanelFrame { cfg: itoCfg; emblem: "flame"; opened: root.opened; alert: root.batteryPresent && root.discharging && root.batteryFraction < 0.15; memos: ["The light is going.", "The lights flicker when it is close.", "The lamp dims. The fog does not."] }
