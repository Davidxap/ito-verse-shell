...
Panel {
  ...
  readonly property bool showValues: itoCfg.get("showValues", true) && itoCfg.get("valueStyle", "percent") !== "off"
    && itoCfg.get("content:ito.display", "icon") !== "icon"
  ...
  implicitWidth: button.implicitWidth + (valueText.visible ? valueText.implicitWidth + 2 : 0)
  ...
  // Ito skin: theme artwork for the bar button. The stock button is a
  // static display glyph; the art follows the live brightness level.
  function brightnessArt() {
    return brightnessPercent >= 50 ? "assets/brightness-high.png" : "assets/brightness-low.png"
  }
  ...
  BarIconButton {
    ...
    bar: root.bar
    text: ""
    iconComponent: brightnessArtComponent
  ...
  Ito.ItoValue {
    ...
    visible: root.showValues && root.brightnessAvailable
    barSize: root.barSize
    label: "BRT"
    vertical: root.bar ? root.bar.vertical === true : false
    hot: false
  ...
  Component {
    ...
    Ito.ItoArt {
      ...
      value: Math.max(0, Math.min(1, root.brightnessPercent / 100))
      size: root.glyphSize
      palette: itoCfg
    }
  }
  ...
  KeyboardPanel {
    ...
    Ito.ItoPanelFrame { cfg: itoCfg; emblem: "eye"; opened: root.opened; memos: ["The town looks different from here.", "Same room. Same door. Again.", "The screen shows a place you already left."] }
