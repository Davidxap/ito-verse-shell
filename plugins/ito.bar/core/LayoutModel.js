...
// Ito-verse holds every one of its own widgets as a movable group, so the extras Shibumi capped at
// two per side are opened up to the room a full bar needs.
var ExtraLimits = { left: 17, center: 23, right: 21 }
...
// Put a group into a region at a slot position, whether or not that slot is empty, shifting its neighbours along
// (into the next empty slot, or a new one at the end) instead of trading places with them. This is what dropping a
// widget "between" two others, or into an empty part of the bar, does. Returns null when it cannot.
function insertGroupAt(value, sourceValue, targetRegionValue, targetIndexValue) {
  var result = copyOrder(value)
  var source = locationFor(result, sourceValue)
  var region = String(targetRegionValue || "")
  var index = Math.floor(Number(targetIndexValue))
  if (!result || !source || Regions.indexOf(region) < 0 || !Number.isFinite(index) || index < 0) return null
  result[source.region][source.index] = ""
  var slots = result[region]
  if (index > slots.length) index = slots.length
  if (index < slots.length && slots[index] === "") {
    slots[index] = source.groupId
  } else {
    var free = -1
    for (var j = index; j < slots.length; j++) {
      if (slots[j] === "") { free = j; break }
    }
    // A blank slot behind the target can be the one just freed above (a reorder within an otherwise
    // full region): without this, dropping an item later in its own full region silently did nothing.
    if (free < 0 && index < slots.length) {
      for (var b = index - 1; b >= 0; b--) {
        if (slots[b] === "") { free = b; break }
      }
    }
    if (free < 0) {
      if (slots.length >= maxCount(region)) return null
      slots.push("")
      free = slots.length - 1
    }
    if (free >= index) {
