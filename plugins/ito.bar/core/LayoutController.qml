...
Item {
  ...
  function insertGroupAt(sourceGroupId, targetRegion, targetIndex) {
    if (v2Mode) {
      const nextSlots = V2LayoutModel.insertGroupAt(
        v2Slots, sourceGroupId, targetRegion, targetIndex)
      return nextSlots ? persistV2Layout(nextSlots) : false
    }
    const currentOrder = currentV1Order()
    const nextOrder = LayoutModel.insertGroupAt(
      currentOrder, sourceGroupId, targetRegion, targetIndex)
    if (!nextOrder) return false
    const nextSplits = LayoutModel.resizeSplits(currentV1Splits(currentOrder), nextOrder)
    return nextSplits ? persist(nextOrder, nextSplits) : false
  }
  ...
  // A style that knows its own default arrangement (itoverse does) resets to that instead of to
  // Shibumi's empty G1..G18 grid, which would drop every widget out of the bar.
  function styleArrangement() {
    const tokens = bar && bar.visualTokens ? bar.visualTokens : null
    const arrangement = tokens ? tokens.defaultArrangement : null
    return arrangement && Array.isArray(arrangement.left)
      && Array.isArray(arrangement.center) && Array.isArray(arrangement.right)
      ? arrangement : null
  }

  function resetToStyleArrangement(arrangement) {
    const dynamic = function(ids) {
      return ids.map(function(id) { return LayoutModel.dynamicGroupId(id) })
    }
    if (v2Mode) {
      const next = V2LayoutModel.copy(V2LayoutModel.defaultLayout())
      const groups = {
        left: dynamic(arrangement.left),
        center: dynamic(arrangement.center),
        right: dynamic(arrangement.right)
      }
      for (const region of ["left", "center", "right"])
        next[region] = next[region].filter(function(id) { return id !== "" }).concat(groups[region])
      for (const region of ["left", "center", "right"]) {
        const floor = V2LayoutModel.Limits[region].min
        while (next[region].length < floor) next[region].push("")
      }
      return persistV2Layout(next)
    }
    const base = LayoutModel.defaultOrder()
    const order = {
      left: base.left.concat(dynamic(arrangement.left)),
      center: base.center.concat(dynamic(arrangement.center)),
      right: base.right.concat(dynamic(arrangement.right))
    }
    return persist(order, LayoutModel.resizeSplits(LayoutModel.defaultSplits(), order))
  }

  function resetLayout() {
    if (mutationBusy) return false
    const arrangement = styleArrangement()
    if (arrangement) return resetToStyleArrangement(arrangement)
