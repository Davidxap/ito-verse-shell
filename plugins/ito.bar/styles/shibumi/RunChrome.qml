...
Item {
  ...
  readonly property real notchBodyRadius: shellStyle === "notch"
    ? (bar.visualTokens.notchBodyRadius !== undefined ? bar.visualTokens.notchBodyRadius : 9) : 0
  ...
  readonly property bool flushToEdge: bar.visualTokens.flushEdge === true
  ...
  Repeater {
    ...
    delegate: Rectangle {
      ...
      // Flush with the screen edge: the side that touches it is square, so the plate reaches the edge with no wedge
      // of wallpaper in the corners.
      topLeftRadius: root.flushToEdge && root.atTop ? 0 : radius
      topRightRadius: root.flushToEdge && root.atTop ? 0 : radius
      bottomLeftRadius: root.flushToEdge && !root.atTop ? 0 : radius
      bottomRightRadius: root.flushToEdge && !root.atTop ? 0 : radius
      ...
      // A style may paint the run itself (the itoverse plate); the rectangle then stays see-through.
      Loader {
        id: runPlateLoader
        anchors.fill: parent
        active: root.bar.visualTokens.runPlate !== undefined
        sourceComponent: root.bar.visualTokens.runPlate
      }

      // The plate takes the run's own corner, so Corners changes it (it used to be a full pill whatever was chosen).
      Binding {
        target: runPlateLoader.item
        property: "cornerRadius"
        value: island.radius
        when: runPlateLoader.item !== null && "cornerRadius" in runPlateLoader.item
      }

      Binding {
        target: runPlateLoader.item
        property: "flatTop"
        value: root.flushToEdge && root.atTop
        when: runPlateLoader.item !== null && "flatTop" in runPlateLoader.item
      }
  ...
  // A style may paint the screen-edge forms as well (the itoverse plate). The shapes below still
  // draw, but their fill is the style's own background, so a see-through style shows this instead.
  Loader {
    id: formPlate
    anchors.fill: parent
    active: root.shellStyle !== "shibumi" && root.shellStyle !== "notch"
      && root.bar.visualTokens.runPlate !== undefined
    sourceComponent: root.bar.visualTokens.runPlate
    z: -1
  }

  // Notch draws its own outline (two shoulders flowing out of the screen edge), so a style's plate is laid over
  // the whole bar and then cut to that outline. Without this the notch showed only the see-through fill of the
  // style's background and the bar seemed to have no plate at all.
  readonly property bool notchPlated: shellStyle === "notch" && bar.visualTokens.runPlate !== undefined

  ShaderEffectSource {
    id: notchOutline
    sourceItem: notchShape
    hideSource: root.notchPlated
    live: true
    visible: false
    width: root.width
    height: root.height
  ...
  Loader {
    id: notchPlate
    anchors.fill: parent
    active: root.notchPlated
    sourceComponent: root.bar.visualTokens.runPlate
    z: -1
    layer.enabled: root.notchPlated
    layer.effect: MultiEffect {
      maskEnabled: true
      maskSource: notchOutline
  ...
  Binding {
    target: notchPlate.item
    property: "cornerRadius"
    value: 0
    when: notchPlate.item !== null && "cornerRadius" in notchPlate.item
  }

  Binding {
    target: formPlate.item
    property: "cornerRadius"
    value: root.shellStyle === "full" ? 0 : root.cornerRadius
    when: formPlate.item !== null && "cornerRadius" in formPlate.item
  }

  // Fit sits against the top edge of the screen, so only its bottom corners are rounded; so does a Dock that is
  // not floating clear of it.
  Binding {
    target: formPlate.item
    property: "flatTop"
    value: root.shellStyle === "fit" || (root.shellStyle === "dock" && root.flushToEdge)
    when: formPlate.item !== null && "flatTop" in formPlate.item
  }
  ...
  Shape {
    ...
    ShapePath {
      strokeColor: "transparent"
      strokeWidth: 0
      // With a style plate the shape is only the outline the plate is cut to, so it is opaque here.
      fillColor: root.notchPlated ? "black" : root.bar.background
      capStyle: ShapePath.FlatCap
      joinStyle: ShapePath.RoundJoin
      startX: 0.5
