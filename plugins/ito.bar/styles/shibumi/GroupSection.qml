...
Item {
  ...
  // A style can ask for plain editing: widgets are dragged onto one another and trade places, with no
  // grid of empty slots, no add or remove buttons. Shibumi's own slot editor stays the default.
  readonly property bool plainEditing: bar.visualTokens
    && bar.visualTokens.plainEditing === true
  ...
  readonly property int maxSlotCount: bar.layoutController
    ...
    : 0
  readonly property bool canAddSlot: slotEditing && !plainEditing
      ...
      Repeater {
        ...
        delegate: Item {
          ...
          readonly property bool emptySlot: root
            ? root.slotEditing && modelData === "" && !root.plainEditing : false
          ...
          readonly property bool proxySlot: root
            ? root.v1Editing && modelData !== "" && !contentShown
              && !root.plainEditing : false
          ...
          // Plain editing: every widget wears a faint outline, and it lights up under the pointer.
          Rectangle {
            visible: root ? root.plainEditing && root.editing && horizontalCell.contentShown : false
            x: groupSlot.x - 3
            y: groupSlot.y - 1
            width: groupSlot.width + 6
            height: groupSlot.height + 2
            radius: root ? root.bar.visualTokens.pillRadius : 0
            color: dragMouse.containsMouse
              ? Qt.rgba(root.bar.urgent.r, root.bar.urgent.g, root.bar.urgent.b, 0.16) : "transparent"
            border.width: 1
            border.color: dragMouse.containsMouse ? root.bar.urgent
          ...
          Rectangle {
            ...
            visible: root && root.layoutSession
          ...
          // Direct dragging, with no edit mode: press on any widget and move, and it comes away from the bar and
          // follows the pointer; let go over another widget and they trade places, let go anywhere else and it
          // goes back. This is a pointer handler, not a mouse area, so a plain click still reaches the widget
          // and the widget's own gestures keep working; the drag only takes over once the pointer has moved.
          DragHandler {
            id: gestureDrag
            target: null
            acceptedButtons: Qt.LeftButton
            ...
            onActiveChanged: {
              ...
              if (active) {
                const point = horizontalCell.mapToItem(null, centroid.position.x, centroid.position.y)
                root.layoutSession.begin(horizontalCell.modelData, horizontalCell.targetVisual, point.x, point.y)
    ...
    Column {
      ...
      Repeater {
        ...
        delegate: Item {
          ...
          // as wide as the bar is thick, with the widget centred in it, so a narrow one is not left at the edge
          implicitWidth: groupHasContent ? Math.max(groupSlot.implicitWidth, root ? root.bar.barSize : 0) : 0
