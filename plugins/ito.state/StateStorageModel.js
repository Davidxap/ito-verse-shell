...
var pluginId = "ito.state"
var schemaKey = "itoStateSchemaVersion"
...
function parse(raw) {
  ...
  if (!found || found[schemaKey] !== 1 || !plain(found.ito)
      || found.ito.version !== 1) return null
  return copy(found)
}
...
function replacement(entry, before, settings) {
  ...
  result.id = pluginId
  result[schemaKey] = 1
  result.ito = mergeSettings(entry.ito, before, settings)
  return result
}
