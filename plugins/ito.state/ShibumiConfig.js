...
function normalizedOrder(value) {
  ...
  if (!Array.isArray(value.left)
      || value.left.length < 7 || value.left.length > 24) return null
  if (!Array.isArray(value.center)
      || value.center.length < 1 || value.center.length > 24) return null
  if (!Array.isArray(value.right)
      || value.right.length < 7 || value.right.length > 28) return null
