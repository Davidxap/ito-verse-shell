...
QtObject {
  ...
  readonly property string suiteVersion: "0.1.1-beta.14.1-ito.1"
  readonly property var providerIds: [
    "ito.state", "hancore.shibumi.control-center",
  ...
  readonly property bool hasActiveBar: _selected("ito.bar") !== null
  readonly property var visibilityBarOwner: {
    const lease = _selected("ito.bar")
  ...
  function noteHostWidgetResolutionExhausted(owner, processId, evidenceValue) {
    ...
    console.warn(JSON.stringify({
      event: "ito-widget-resolution-exhausted",
      processId: pid,
  ...
  function providerRecord(id, owner, host, manifest, version) {
    ...
    const kind = key === "ito.bar" ? "bar" : "service"
